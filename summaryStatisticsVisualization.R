# Sales Order Data --------------------------------------------------------
p <- ggplot(sales_order_summary_cid_withLabel, aes(log(mean_gmv), fill=custom_label))
p + geom_density(alpha=0.25) + ggtitle("Average Spending and Fraud") + labs(x = "Log(mean_gmv)")  + guides(fill=guide_legend(title="Customer Label"))

p <- ggplot(sales_order_summary_cid_withLabel, aes(log(sum_gmv), fill=custom_label))
...
p <- ggplot(sales_order_summary_cid_withLabel, aes(log(mean_diff_purc), fill=custom_label))
p + geom_density(alpha=0.25) + ggtitle("Time inBetween Transactions and Fraud") + labs(x = "Log(seconds)") + guides(fill=guide_legend(title="Customer Label"))

p <- ggplot(sales_order_summary_cid_withLabel, aes(log(num_purc), fill=custom_label))
p + geom_density(alpha=0.25) + ggtitle("Number of Transactions and Fraud") + labs(x = "Log(Number of Purchases)") + guides(fill=guide_legend(title="Customer Label"))

p <- ggplot(sales_order_summary_cid_withLabel, aes(log(unicat), fill=custom_label))
p + geom_density(alpha=0.25) + ggtitle("Number of Distinct Categories and Fraud") + labs(x = "Log(Number of Distinct Categories)") + guides(fill=guide_legend(title="Customer Label"))
